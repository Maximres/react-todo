...
const ReminderInput = forwardRef(({isOpen, setIsOpen}: Props, ref: React.Ref<HTMLDivElement>) => {
    ...
    const reminderText = hasReminder ? `Remind me at ${format(selectedTask.remindDate as Date, pattern12AmPmFormat)}` : 'Remind me';
    const reminderDetailedText = hasReminder ? format(selectedTask.remindDate as Date, weekMonthDayFormat) : null;
